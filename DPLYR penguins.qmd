...
```{r}
penguins %>%
  relocate(is.factor, .after = is.integer)
```
...
```{r}
penguins %>%
  mutate(mass_1b = body_mass_g * 0.0022)
```
...
penguins %>%
  mutate(species = as.character(species), 
         flipper_cm = flipper_length_mm * .1, 
...
penguins %>%
  ...
  summarize(across(contains("length"), mean, na.rm = TRUE, .names = "{.col}_max"))
