...
average_species_flipper_length <- penguins_data_from_csv %>% 
  group_by(species) %>% 
  summarise(average_species_flipper_length = mean(flipper_length_mm, na.rm = TRUE))
...
```{r}
ggplot(data = average_species_flipper_length, aes(x = species, y = average_species_flipper_length, fill = species)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  scale_fill_viridis_d() +
  coord_flip() +
  theme(legend.position = "none") +
  labs(title = "Palmer Penguins, Average Flipper Length by Species",
       subtitle = "3 penguin species observed on three islands in the Palmer Archipelago, Antarctica",
       x = "average flipper length (mm)",
       y = "species")
...
```{r}
interactive_box_plot <-ggplot(data = penguins_data_from_csv, aes(x = species, y = body_mass_g, fill = species)) +
  geom_boxplot() +
  theme_bw() +
  scale_color_viridis_d() +
...
-   How many penguins live on Torgersen Island?
    ...
    ```{r}
    table(penguins_data_from_csv$island)
    ```

Answer: 52 penguins live on Torgersen island
...
-   Create a dataframe that has the average culmen depth for each species
    ...
    ```{r}
    average_species_culmen_depth <- penguins_data_from_csv %>% 
      group_by(species) %>% 
      summarise(average_species_culmen_depth = mean(culmen_depth_mm, na.rm = TRUE))
...
-   <div>
    ...
    ```{r}
    ggplot(data = average_species_culmen_depth, aes(x = species, y = average_species_culmen_depth, fill = species)) +
      geom_bar(stat = "identity") +
      theme_bw() +
      scale_fill_viridis_d() +
      coord_flip() +
      theme(legend.position = "none") +
      labs(title = "Palmer Penguins, Average Culmen Depth by Species",
           subtitle = "3 penguin species observed on three islands in the Palmer Archipelago, Antarctica",
           x = "species",
           y = "average culmen depth (mm)")
    ```
